...
class Organizer extends Stakeholder {
  ...
  update(eventState) {
    ...
    console.log(
      `Organizer ${this.name} received an update for event: ${eventState.getName()}`,
    );
    console.log(`Event details: ${eventState.getDescription()}`);

    // Check if this is an event organized by this organizer
    const affectedEvent = this.organizedEvents.find(
      (event) => event.getState().getName() === eventState.getName(),
    );

    if (affectedEvent) {
      console.log(
        `This is one of your organized events. Status: ${affectedEvent.status}`,
      );
    }
  }
  ...
  addOrganizedEvent(event) {
    if (!this.organizedEvents.includes(event)) {
      ...
      console.log(
        `Event "${event.getState().getName()}" added to organized events.`,
      );
    }
  }
  ...
  removeOrganizedEvent(event) {
    ...
    if (index !== -1) {
      ...
      console.log(
        `Event "${event.getState().getName()}" removed from organized events.`,
      );
      return true;
    }
    return false;
