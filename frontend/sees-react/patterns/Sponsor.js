...
class Sponsor extends Stakeholder {
  ...
  update(eventState) {
    console.log(
      `Sponsor ${this.name} received an update for event: ${eventState.getName()}`,
    );
    console.log(`Event details: ${eventState.getDescription()}`);

    // Check if this is an event sponsored by this sponsor
    const sponsoredEvent = this.sponsoredEvents.find(
      (event) => event.event.getState().getName() === eventState.getName(),
    );

    if (sponsoredEvent) {
      console.log(
        `This is an event you're sponsoring with $${sponsoredEvent.amount}.`,
      );
    }
  }
  ...
  sponsorEvent(event, amount = null) {
    ...
    const sponsorshipAmount =
      amount !== null ? amount : this.contributionAmount;
    ...
    const existingSponsorhip = this.sponsoredEvents.find(
      (s) => s.event === event,
    );
    if (existingSponsorhip) {
      console.log(
        `Already sponsoring this event with $${existingSponsorhip.amount}.`,
      );
      return false;
    }
    ...
    console.log(
      `${this.name} is now sponsoring ${event.getState().getName()} with $${sponsorshipAmount}.`,
    );
    return true;
  }
  ...
  provideFunding(event, amount) {
    ...
    console.log(
      `${this.name} has provided $${amount} funding for ${event.getState().getName()}.`,
    );
    console.log(
      `Total funding provided to date: $${this.totalFundingProvided}`,
    );
